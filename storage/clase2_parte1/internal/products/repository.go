...
const (
	GetProduct    string = "SELECT p.id, p.name, p.type, p.count, p.price FROM products p WHERE id = ?"
	InsertProduct string = "INSERT INTO products(name, type, count, price) VALUES( ?, ?, ?, ? )"
	UpdateProduct string = "UPDATE products SET name = ?, type = ?, count = ?, price = ? WHERE id = ?"
...
func (r *repository) GetOneWithcontext(ctx context.Context, id int) (domain.Product, error) {
	var product domain.Product
	rows, err := r.db.QueryContext(ctx, GetProductSleep, id)
	if err != nil {
		return product, err
	}
	for rows.Next() {
