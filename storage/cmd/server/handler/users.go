...
func (u *User) GetById() gin.HandlerFunc {
	return func(ctx *gin.Context) {
		...
		user, err := u.service.GetById(ctx, idInt)
		if err != nil {
...
func (u *User) GetByName() gin.HandlerFunc {
	return func(ctx *gin.Context) {
		nombre := ctx.Param("nombre")
		fmt.Println(nombre)
		user, err := u.service.GetByName(ctx, nombre)
		if err != nil {
			ctx.JSON(http.StatusNotFound, web.NewResponse(http.StatusNotFound, nil, err.Error()))
			return
		}
...
func (u *User) Update() gin.HandlerFunc {
	return func(ctx *gin.Context) {
		...
		user, err := u.service.Update(ctx, int(id), req.Nombre, req.Apellido, req.Email, req.Edad, req.Altura, req.Activo, req.FechaCreacion)
		if err != nil {
			ctx.JSON(http.StatusNotFound, web.NewResponse(http.StatusNotFound, nil, err.Error()))
			return
		}
...
func (u *User) Delete() gin.HandlerFunc {
	return func(ctx *gin.Context) {
		...
		err = u.service.Delete(ctx, int(id))
		if err != nil {
			ctx.JSON(http.StatusNotFound, web.NewResponse(http.StatusNotFound, nil, err.Error()))
			return
		}
...
func (u *User) Patch() gin.HandlerFunc {
	return func(ctx *gin.Context) {
		...
		user, err := u.service.Patch(ctx, int(id), req.Apellido, req.Edad)
		if err != nil {
			ctx.JSON(http.StatusNotFound, web.NewResponse(http.StatusNotFound, nil, err.Error()))
			return
		}
