...
type Service interface {
	...
	Update(id int, name string, pType string, price float64, count int) (domain.Product, error)
	Delete(id int) error
}
...
func (s *service) Update(id int, name string, pType string, price float64, count int) (domain.Product, error) {
	uProduct := domain.NewProduct(id, name, pType, price, count)
	updatedProduct, err := s.r.Update(uProduct)
	if err != nil {
		return domain.Product{}, err
	}
