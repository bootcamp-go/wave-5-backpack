...
import (
	...
	"clase2-storage-implementation-tm/pkg/bank/web"
...
func (t *Transaction) GetByName() gin.HandlerFunc {
	return func(ctx *gin.Context) {
		token := ctx.GetHeader("token")
		if token != os.Getenv("TOKEN") {
			ctx.JSON(401, web.NewResponse(401, nil, InvalidToken))
		...
		p, err := t.service.GetByName(codeParam)
		if err != nil {
			ctx.JSON(404, web.NewResponse(404, nil, err.Error()))
			return
		}
...
func (t *Transaction) GetOneWithcontext() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		transaction, err := t.service.GetOneWithContext(c, int(idInt))
		if err != nil {
			web.Error(c, http.StatusNotFound, "error: %s", err.Error())
			return
		}
		web.Success(c, http.StatusOK, transaction)
	}
}
