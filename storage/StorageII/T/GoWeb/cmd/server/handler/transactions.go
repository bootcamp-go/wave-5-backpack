...
func (tt *Transaction) GetByIdCtx() gin.HandlerFunc {
	return func(ctx *gin.Context) {
		...
		tran, err := tt.service.GetByIdCtx(ctx, id)
		if err != nil {
			ctx.JSON(500, web.NewResponse(500, nil, err.Error()))
		}
		ctx.JSON(200, web.NewResponse(200, tran, ""))
...
func (tt *Transaction) UpdateCtx() gin.HandlerFunc {
	return func(ctx *gin.Context) {
		...
		tran, err := tt.service.UpdateCtx(ctx, id, tr.Code, tr.Coin, tr.Amount, tr.Emisor, tr.Receptor, tr.Date)
		if err != nil {
			ctx.JSON(404, web.NewResponse(404, nil, err.Error()))
		}
